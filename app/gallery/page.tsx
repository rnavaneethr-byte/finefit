  ...
  return (
    <>
      ...
      <section className="relative h-[60vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white px-4 animate-fadeInUp">
            <p className="section-subtitle scroll-reveal" style={{ color: '#C8A16F' }}>Our Work</p>
            <h1 className="font-serif text-5xl md:text-6xl scroll-reveal">Gallery</h1>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 max-w-4xl mx-auto text-center">
        ...
        <p className="text-lg text-gray-600 leading-relaxed scroll-reveal">
          ...
          {galleryImages.map((image, idx) => (
            <div key={idx} className="group relative overflow-hidden aspect-[3/4] cursor-pointer rounded-lg shadow-lg hover-lift animate-fadeInUp" style={{ animationDelay: `${idx * 0.05}s` }}>
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                <p className="text-xs uppercase tracking-widest font-semibold mb-2 scroll-reveal" style={{ color: '#C8A16F' }}>
                  {image.category}
                </p>
                <h3 className="text-lg font-serif text-white leading-tight scroll-reveal">{image.alt}</h3>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="py-32 px-4 bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 animate-fadeInUp">
            <p className="section-subtitle scroll-reveal">Collections</p>
            <h2 className="section-title scroll-reveal">Browse by Category</h2>
          </div>
          <div className="grid md:grid-cols-4 gap-6">
            {['Suits', 'Shirts', 'Formalwear', 'Workshop'].map((category, idx) => (
              <div key={idx} className="relative overflow-hidden bg-white rounded-lg p-8 text-center hover-lift group animate-fadeInUp shadow-md" style={{ animationDelay: `${idx * 0.1}s` }}>
                ...
                <h3 className="font-serif text-2xl text-gray-900 mb-2 transition-colors duration-300 scroll-reveal" style={{ '--hover-color': '#C8A16F' } as any}>{category}</h3>
                <p className="text-gray-600 text-sm transition-colors duration-300 scroll-reveal" style={{ '--hover-color': '#C8A16F' } as any}>Explore Collection →</p>
              </div>
            ))}
          </div>
        </div>
